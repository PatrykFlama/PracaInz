  ...
  \begin{columns}[c]
    ...
    \begin{center}
      \begin{tikzpicture}[
        ...
        \node[state, accepting] (q1) at (2.2,0) {$q_1$};
        \node[state] (q2) at (4.4,0) {$q_2$};
...
\only<1>{
    \begin{figure}
        \includegraphics[width=0.6\linewidth]{images/len.png}
    \end{figure}
}

\only<2>{
    \begin{figure}
        \includegraphics[width=0.6\linewidth]{images/nsam.png}
    \end{figure}
}

\only<3>{
    \begin{figure}
        \includegraphics[width=0.6\linewidth]{images/medge.png}
    \end{figure}
}
\end{frame}
...
\begin{block}{Problem naprawienia częściowego DFA jest $\mathbb{NP}$-trudny}
...
Weźmy instancję problemu \textit{najmniejszego zgodnego automatu} --- liczbę naturalną $n$ oraz zbiory słów $S^+$ oraz $S^-$. Stwórzmy częściowy automat DFA $A = (Q, \Sigma, \delta, q_\lambda, \mathbb{F_A}, \mathbb{F_R})$, gdzie $|Q| = n$,
dla wszystkich stanów $q \in Q$ oraz liter $a \in \Sigma$ przejście $\delta(q, a)$ jest nieokreślone oraz \underline{$\mathbb{F_A} = \emptyset$ i $\mathbb{F_R} = \emptyset$},
