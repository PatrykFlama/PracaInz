...
% TODO: fix sink+ were we to lead all pi to it, then we would accept all samples (and all positive); we have 2 options:
% 1. in sink+ discard words (suffixes) having with more than 1 test edge
% 2. create rejecting samples, that ensure pi will not lead to any positive state

Zagwarantowanie tego faktu opiera się na prostej obserwacji: jedyne krawędzie $t$, które nie prowadzą do stanu odrzucającego, znajdują się za wierzchołkami $v_i$. Oznacza to, że aby słowo zostało zaakceptowane, musimy przejść przez krawędź $p_x$ do któregoś ze stanów $v_i$, a następnie przez krawędź $t$ do stanu akceptującego. \\
W takim przypadku, aby zagwarantować przejście krawędzią $p_x$ do któregoś ze stanów $v_i$, dodajemy w każdej próbce literę $\text{test}$ zaraz po literze $p_x$. W ten sposób, aby słowo zostało zaakceptowane, musimy przejść krawędzią $p_x$ do któregoś ze stanów $v_i$, a następnie krawędzią $\text{test}$ do stanu akceptującego. \\
