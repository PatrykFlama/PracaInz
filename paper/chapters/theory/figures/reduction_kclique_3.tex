...
\begin{tikzpicture}[
    ...
    \draw[->,thick] (s) -- (sink) node[midway,right] {choice};
    ...
    % krawędzie p
    \draw[->,dashed,thick] (s) -- (t1) node[midway,above] {$bin(1)$};
    \node at (2.8,-1.2) {$\vdots$};
    \draw[->,dashed,thick] (s) -- (tx) node[midway,above] {$bin(x)$};
    \draw[->,dashed,thick] (s) -- (ty) node[midway,above] {$bin(y)$};
    \node at (2.8,-2.6) {$\vdots$};
    \draw[->,dashed,thick] (s) -- (tlast) node[midway,above] {$bin(k)$};

    % sekcja weryfikacji wierzchołka
    \node[draw,state,inner sep=2pt] (vv1) at (6.5,-0.2) {$q_{\lambda}$};
    \node (vdots1) at (6.5,-0.8) {$\vdots$};
    \node[draw,state,inner sep=2pt] (vvi) at (6.5,-1.6) {$u_{i}$};
    \node (vdots1) at (6.5,-2.2) {$\vdots$};
    \node[draw,state,inner sep=2pt] (vvn) at (6.5,-3) {$q_{\lambda}$};
    ...
    % sekcja wyboru
    \node[draw,state,inner sep=2pt] (choicenode) at (9,-1.6) {$ $};
    \draw[->,thick] (vvi) -- (choicenode) node[midway,above] {$\text{choice}$};
    \node[draw,state,inner sep=2pt] (choice1) at (10.5,-0.8) {$q_{\lambda}$};
    \node[draw,state,inner sep=2pt] (choice2) at (10.5,-2.4) {$c_2$};
    ...
    \node[accept, inner sep=1.5pt] (accept) at (13.5,-1.4) {};
    \node[reject, inner sep=1.5pt] (reject) at (13.5,-3) {};
