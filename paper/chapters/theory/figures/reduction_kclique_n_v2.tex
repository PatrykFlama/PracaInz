...
\begin{tikzpicture}[
    ...
    % krawędź choice
    \node[draw,state,inner sep=2pt] (choice_edge) at (6,-1.6) {$ $};
    \draw[->,thick] (vi) -- (choice_edge) node[midway,above] {$\text{choice}$};

    % sekcja weryfikacji wierzchołka
    \node[draw,state,inner sep=2pt] (vv1) at (7.5,-0.2) {$q_{\lambda}$};
    \node (vdots1) at (7.5,-0.8) {$\vdots$};
    \node[draw,state,inner sep=2pt] (vvi) at (7.5,-1.6) {$u_i$};
    \node (vdots1) at (7.5,-2.2) {$\vdots$};
    \node[draw,state,inner sep=2pt] (vvn) at (7.5,-3) {$q_{\lambda}$};

    \draw[->,thick] (choice_edge) -- (vv1) node[midway,above] {$e_1$};
    \draw[->,thick] (choice_edge) -- (vvi) node[midway,above] {$e_i$};
    \draw[->,thick] (choice_edge) -- (vvn) node[midway,above] {$e_n$};

    % sekcja wyboru
    \node[draw,state,inner sep=2pt] (choice1) at (10,-0.8) {$q_{\lambda}$};
    \node[draw,state,inner sep=2pt] (choice2) at (10,-2.4) {$c_2$};
    ...
    \node[accept, inner sep=1.5pt] (accept) at (13,-1.4) {};
    \node[reject, inner sep=1.5pt] (reject) at (13,-3) {};
    ...
    \node[
        ...
        thick,
        rounded corners,
        fit=(vv1)(vvi)(vvn)(choice1)(choice2)(accept)(reject)(choice_edge),
