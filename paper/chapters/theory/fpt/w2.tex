...
Utworzymy teraz przypisanie brakujących krawędzi w automacie z rysunku \ref{fig:ksetcover}, tak aby powstał poprawny pełny automat deterministyczny oraz aby $\forall s \in S^+$ było akceptowane, a $\forall s \in S^-$ odrzucane.
...
Załóżmy, że istnieje rozwiązanie problemu naprawienia $k$-częściowego DFA, czyli istnieje przypisanie brakujących krawędzi w automacie z rysunku \ref{fig:ksetcover}, takie że powstał poprawny pełny automat deterministyczny oraz aby $\forall s \in S^+$ było akceptowane, a $\forall s \in S^-$ odrzucane.
...
Ponadto, z próbek \ref{eq:kcover-cover} wynika, że krawędź $p_x$ nie może prowadzić do stanu $q_\lambda$, ani $\forall i \in \{1..n\}$ do stanu $\delta(s_i, t)$, ponieważ w obu przypadkach zawsze następna litera to $t$, która prowadzi do stanu odrzucającego $\text{sink}_-$. \\
...
Załóżmy, że istnieje rozwiązanie problemu naprawienia $k$-częściowego automatu binarnego, czyli istnieje przypisanie brakujących krawędzi w automacie z rysunku \ref{fig:ksetcover2}, takie że powstał poprawny pełny automat deterministyczny oraz aby $\forall s \in S^+$ było akceptowane, a $\forall s \in S^-$ odrzucane.
...
Dodatkowo, nie może ona prowadzić do żadnego stanu $\forall i \in \{1..n\} \ e_i$, ponieważ następna litera w próbce to $1$, która w konstrukcji automatu prowadzi do stanu odrzucającego $\text{sink}_-$. \\
Jeżeli brakująca krawędź prowadzi do stanu $\forall i \in \{1..n\} \ \forall j \in \{1..10\} \ \delta(s_i, 1^{j})$, to w próbce następuje po niej ciąg liter $[1^{10} \ 0]$, który będzie prowadzić do stanu odrzucającego $\text{sink}_-$. \\
Oznacza to też, że nie może ona prowadzić do żadnego stanu $\forall i \in \{1..k\} \ p_i$, ponieważ zawsze następujący ciąg liter to $[ 1^{10} \ 0]$, który zawsze prowadzi do stanu odrzucającego $\text{sink}_-$.
