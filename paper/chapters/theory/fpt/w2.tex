...
Próbki \ref{eq:kcover-cover} będą akceptowane, ponieważ dla każdego elementu $u_j \in U$ istnieje zbiór $S_{i_x} \in \mathcal{F}'$ taki że $u_j \in S_{i_x}$, a zatem po pewnej liczbie przejść podsłowami $\text{dla } y \in \{1,\dots,x-1\} \ [p_y \ t \ e_j]$ prowadzącymi do stanu $q_{\lambda}$, przejdziemy podsłowem $[p_x \ t \ e_j]$, gdzie $p_x$ prowadzi do stanu $s_{i_x}$, więc następujące krawędzie $[t \ e_j]$ doprowadzą do stanu akceptującego $\text{sink}_+$ (ponieważ, z~konstrukcji automatu, $u_j \in S_{i_x} \implies \delta(\delta(s_{i_x}, t), e_j) = \text{sink}_+$).

Próbki \ref{eq:kcover-si} będą odrzucane, ponieważ każda brakująca krawędź $p_x$ prowadzi do stanu $s_i$ dla pewnego $i \in \{1,\dots,n\}$, a z~konstrukcji automatu $s_i \in \mathbb{F_R}$.
...
Ponadto, z~próbek \ref{eq:kcover-cover} wynika, że krawędź $p_x$ nie może prowadzić do stanu $q_\lambda$, ani $\text{dla każdego } i \in \{1,\dots,n\}$ do stanu $\delta(s_i, t)$, ponieważ w obu przypadkach zawsze następna litera to $t$, która prowadzi do stanu odrzucającego $\text{sink}_-$. 
...
% Analogicznie do litery $t$ w konstrukcji z rysunku \ref{fig:ksetcover}, korzystamy z ciągu liter $1^{10}$, aby wymusić prowadzenie brakujących krawędzi do stanów $s_i$.
% Musimy dodać próbki, które wymuszą prowadzenie brakujących krawędzi do początku ciągu liter $1^{10}$ oraz próbki, które wymuszą przejście przez cały ciąg liter $1^{10}$ - w szczególności przez ostatni stan w tym ciągu.

% \begin{equation}
% \text{dla każdego } x\in\{1,\dots,k\} \ {[0^{x}1] \in S^-}
...
Dodatkowo, nie może ona prowadzić do żadnego stanu $\text{dla } i \in \{1,\dots,n\} \ e_i$, ponieważ następna litera w~próbce to $1$, która w~konstrukcji automatu prowadzi do stanu odrzucającego $\text{sink}_-$. 
Jeżeli brakująca krawędź prowadzi do stanu $\text{dla } i \in \{1,\dots,n\} \ \text{dla } j \in \{1,\dots,10\} \ \delta(s_i, 1^{j})$, to w~próbce następuje po niej ciąg liter $[1^{10} \ 0]$, który będzie prowadzić do stanu odrzucającego $\text{sink}_-$. 
Oznacza to też, że nie może ona prowadzić do żadnego stanu $\text{dla } i \in \{1,\dots,k\} \ p_i$, ponieważ zawsze następujący ciąg liter to $[ 1^{10} \ 0]$, który zawsze prowadzi do stanu odrzucającego $\text{sink}_-$.
