...
\begin{proof}
    ...
    Maszyna turinga zgaduje $k$ stanów docelowych $q \in Q$ wybrakowanych przejść w $O(k \cdot \log |Q|)$ krokach niedeterministycznych. Następnie deterministycznie weryfikuje, czy zgadnięte uzupełnienie powoduje akceptację wszystkich słów z $S^+$ i odrzucenie wszystkich słów z $S^-$. Tą weryfikację wykonuje w czasie wielomianowym względem rozmiaru wejścia - dla przykładu symulując działanie automatu.
